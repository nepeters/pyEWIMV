...
rad = np.max(tree.query(qgrid,k=3)[0])
...
#        fibre_q[fi][yi] = qfib[fib_idx].reshape((len(fz_idx[0]),4))
        ...
        """ find tube """
        tube = (temp <= rad)
        temp = np.column_stack((np.argwhere(tube),temp[tube]))
        
        """ round very small values """
        temp[:,2] = np.round(temp[:,2], decimals=7)
        
        """ move values at zero to very small (1E-5) """
        temp[:,1] = np.where(temp[:,2] == 0, 1E-5, temp[:,1])
        
        """ sort by min distance """
        temp = temp[np.argsort(temp[:,1],axis=0)]
        
        """ run through list of unique fibre sites, gen list of OD cells/dist """
        
        nn_gridPts[fi][yi] = {}
        nn_gridDist[fi][yi] = {}
        
        for fib_pt in range(tube.shape[1]):
            
            nn_gridPts[fi][yi][fib_pt] = temp[temp[:,1] == fib_pt,0]
            nn_gridDist[fi][yi][fib_pt] = temp[temp[:,1] == fib_pt,2]
...
for hi, h in enumerate(hkls):
    ...
    pf_od[hi] = {}
    od_pf[hi] = np.zeros((bungeAngs.shape[0],len()))
    
    for yi in range(len(nn_gridPts[hi].keys())):
        
        pf_od[hi][yi] = {}
        
        for fib_pt in list(nn_gridPts[hi][yi].keys()):
            
            od_cells = nn_gridPts[hi][yi][fib_pt]
            
            #TODO: work on scaling towards max distance
            scaled_dist = nn_gridDist[hi][yi][fib_pt] / rad
            weights = 1 / ( ( scaled_dist )**tube_exp )
            tot_wgts = np.sum(weights)
        
            if np.any(weights < 0): raise ValueError('neg weight')
                
            if np.any(weights == 0): raise ValueError('zero weight')
        
            pf_od[hi][yi][fib_pt] = {'cell': od_cells, 'weight': weights, 'tot_weight': tot_wgts}
