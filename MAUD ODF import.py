...
for g in tqdm(g_betaFiber):
    ...
    vf.append(MAUD_od.compVolume(g,18))
...
# test = R.from_euler('ZXZ', betaFiber[10], degrees=True)
...
# eu_grid = np.array([MAUD_od.phi1.flatten(),MAUD_od.Phi.flatten(),MAUD_od.phi2.flatten()]).T
...
#     mo_cell.append(np.unique(misori[gi],axis=0).T)    
#     # mis_cell.append(np.unique(misori[gi],axis=0)[:,1].T) 
#     # misori.append(np.argmin(np.arccos((np.vstack(trace)-1)/2)))
#     # k+=1

# # mo_cell = np.unique(np.hstack(mo_cell).astype(int))
# mo_cell = np.hstack(mo_cell).T
